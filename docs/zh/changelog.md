...
- 搜索接口收到格式异常的响应、HTTP 408/429、临时服务器错误时会自动重试；论文详情页暂时无法解析时也不再立即失败。
- 使用 HTML 解析替代脆弱的 citation-meta 正则表达式，兼容属性顺序变化，并正确还原标题、作者、摘要和发表信息中的网页转义字符。
- 搜索结果会保留 `h0065` 等 Historical Paper 系列编号，MCP 论文详情也支持这些编号，不再错误返回 `w0000` 或直接拒绝。
- 所有重试均失败后，搜索接口会返回可理解的具体错误信息。
